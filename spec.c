...
make_call_no_arg_return_s32() {
  Function_Builder builder = fn_begin();
  {
    ...
    *returns = (const Value){0};
    ...
    fn_return(&builder, call_function_value(&builder, arg0, 0, 0));
  }
  return fn_end(&builder);
}
...
make_partial_application_s64(
  ...
  Function_Builder builder = fn_begin();
  {
    fn_return(&builder, call_function_value(&builder, original_fn, value_from_s64(arg), 1));
  }
  return fn_end(&builder);
}
...
spec("mass") {
  ...
  it("should create function that returns s64 value that was passed") {
    ...
    s64 result = value_as_function(id_value, fn_type_s64_to_s64)(42);
    check(result == 42);
  }
