...
spec("function") {
  ...
  describe("plus") {
    it("should fold s8 immediates and move them to the result value") {
      Value *reg_a = value_register_for_descriptor(temp_context, Register_A, &descriptor_s8, (Source_Range){0});
      plus(
        temp_context, &test_range, reg_a,
        value_from_s8(temp_context, 30, (Source_Range){0}),
        value_from_s8(temp_context, 12, (Source_Range){0})
      );
      check(dyn_array_length(builder->code_block.instructions) == 1);
      Instruction *instruction = dyn_array_get(builder->code_block.instructions, 0);
      check(instruction_equal(
    ...
    it("should move `a` to result and add `b` to it when result is neither `a` or `b`") {
      Value *reg_a = value_register_for_descriptor(temp_context, Register_A, &descriptor_s32, (Source_Range){0});
      Value *reg_b = value_register_for_descriptor(temp_context, Register_B, &descriptor_s32, (Source_Range){0});
      Value *reg_c = value_register_for_descriptor(temp_context, Register_C, &descriptor_s32, (Source_Range){0});
    ...
    it("should use the larger operand's size for the result") {
      Value *r32 = value_register_for_descriptor(
        temp_context, register_acquire_temp(builder), &descriptor_s32, (Source_Range){0}
      );
      ...
      check(dyn_array_length(builder->code_block.instructions) == 3);
      Value *temp = value_register_for_descriptor(
        temp_context, register_acquire_temp(builder), &descriptor_s32, (Source_Range){0}
      );
      check(instruction_equal(
        dyn_array_get(builder->code_block.instructions, 0),
    ...
    it("should use a temp register when result is also `a`, but both operands are memory") {
      Value *temp_reg = value_register_for_descriptor(temp_context, Register_C, &descriptor_s32, (Source_Range){0});
  ...
  describe("minus") {
    it("should fold s8 immediates and move them to the result value") {
      Value *temp_reg = value_register_for_descriptor(temp_context, Register_C, &descriptor_s8, (Source_Range){0});
      minus(
        temp_context, &test_range, temp_reg,
        value_from_s8(temp_context, 52, (Source_Range){0}),
        value_from_s8(temp_context, 10, (Source_Range){0})
      );
      check(dyn_array_length(builder->code_block.instructions) == 1);
      Instruction *instruction = dyn_array_get(builder->code_block.instructions, 0);
      check(instruction_equal(
    ...
    it("should use a temp A register when result is also `b`") {
      Value *temp_reg = value_register_for_descriptor(temp_context, Register_D, &descriptor_s32, (Source_Range){0});
      Value *reg_b = value_register_for_descriptor(temp_context, Register_B, &descriptor_s32, (Source_Range){0});
